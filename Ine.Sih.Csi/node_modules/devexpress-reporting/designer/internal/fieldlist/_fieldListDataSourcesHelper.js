...
var FieldListDataSourcesHelper = (function () {
    ...
    FieldListDataSourcesHelper.prototype._updateInnerCache = function (dsId, currentPath, result) {
        var _this = this;
        result.forEach(function (item) {
            var itemPath = [currentPath, item.name].join('.');
            if (item.relationPath) {
                Object.defineProperty(item, 'items', {
                    get: function () { return _this._findItems(_this._innerCache[dsId], item.relationPath.split('.')); }
                });
            }
            else if (item.items) {
                _this._updateInnerCache(dsId, itemPath, item.items);
            }
        });
    };
                ...
                if (newRequest['dataSource']) {
                    ...
                    cache[newRequest.fullPath] = fieldsCallback(newRequest).done(function (result) {
                        if (Array.isArray(result)) {
                            ...
                            _this._updateInnerCache(newRequest.id || newRequest.ref, newRequest.fullPath, result);
                            $deferred.resolve(_this._getItemsFromCache(request));
                        }
                        else {
                            $deferred.resolve(result);
